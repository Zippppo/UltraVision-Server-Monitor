...
- can run as a scheduled one-shot refresh, currently every 1 hour via Windows Task Scheduler,
...
For GitHub Pages publishing, a long-running local web server is not required. Use Task Scheduler to run one refresh every 1 hour:

```powershell
schtasks /Create /TN "Storage Monitor Refresh" /SC HOURLY /MO 1 /ST 00:00 /TR "powershell.exe -NoProfile -ExecutionPolicy Bypass -File E:\CODE\Storage-Monitor\scripts\refresh_once.ps1" /F
```
